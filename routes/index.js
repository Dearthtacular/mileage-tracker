...
router.get('/', function(req, res) {
  ...
  //===============================================================================================
  res.render('index') // change back to redirect
}); 
...
router.get('/oauth2callback', passport.authenticate(
  'google',
  {
    successRedirect : '/vehicles', // UPDATE THIS, where do you want the client to go after you login 
    failureRedirect : '/vehicles' //  UPDATE THIS, where do you want the client to go if login fails
  }
));
...
router.get('/logout', function(req, res){
  req.logout(function(){ //< - req.logout comes from passport, and what it does is destorys the cookie keeping track of the user!
    res.redirect('/vehicles') // <---- UPDATE THIS TO WHERE YOU WANT THE USER TO GO AFTER LOGOUT
  })
})
